...
{
  read_head();
  install_trans2(); // if committed, copy from log to disk
  int tail;
...
void
write_to_log(struct buf *b){
  // int t = 0;
  struct buf *leftbuf = bread(log.dev, log.start + log.lh.n + 1);
  memmove(leftbuf->data, b->data, BSIZE);
  bwrite(leftbuf);
  brelse(leftbuf);
}